<?php
    ...
    if (($row = mysqli_fetch_array($resQuery))  == NULL) {
        echo "<h2 style='color: red'>No articles</h2>";
    }
    foreach ($resQuery as $elem) {
        ...
        foreach ($elem as $value) {
            if ($value == $elem['img']) {
                echo "<img src='$value'  width='140px' ";
            } else {
    ...
    if (isset($_POST['delete'])) {
        if ($_POST['delete'] == "Delete") {
            $id = $_POST['value_id'];
            $query = mysqli_query($link, "DELETE FROM `articles` WHERE id = '$id'");
            refresh();
        }
    }
    if (isset($_POST['modify_item'])) {
        if ($_POST['modify_item'] == "Edit") {
            ...
            $description = $_POST['4'];
            $price = $_POST['5'];
            $query = mysqli_query($link, "UPDATE `articles` SET `name` = '$name', `brand` = '$brand', `phone_model` = '$phone_model', `description` = '$description', `price` = '$price' WHERE id = '$id'");
            refresh();
        }
    }
