<?php
    ...
    foreach ($resLogQuery as $elem) {
        echo "<tr>";
        echo "<td style='color: white; height: 70px; font-weight: bold'>" . $elem['name'] . "</td>";
        echo "<td style='color: white; height: 70px;'>" . $elem['price'] . " $</td>";
    ...
    echo "<tr><td style='color: white; height: 70px; font-weight: bold'>Total: </td><td style='color: white; height: 70px;'>$total $</td><td><form method='post'><input style='padding: 10px; margin-top: 10px;' type='submit' name='order' value='Order'></form></td></tr>";
    echo "</table>";
    echo "</div>";
    echo "</div>";
    ...
    if (isset($_POST['order'])) {
        if ($_POST['order'] == 'Order') {
            if (!$_SESSION['loggued_on_user']) {
                ...
                echo "<div><h2 style='color: green'>order sent</h2></div>";
            }
        }
    }
